...
void QuickStuff(int** matrix, int size) {
...
lms_config_ptr SOLSTICE_Decomp(lms_mat_ptr ds_mat_ptr, double delta) {


	int** int_mat_ptr = as_integer_matrix(ds_mat_ptr);
	QuickStuff(int_mat_ptr, ds_mat_ptr->dim);

	lms_mat_ptr mat_ptr = Make_Matrix(ds_mat_ptr->dim, int_mat_ptr);
	Free_Sparse_Matrix(int_mat_ptr, ds_mat_ptr->dim);
